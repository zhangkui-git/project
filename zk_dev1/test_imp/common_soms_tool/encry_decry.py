...
class RsaEncrypt():
    ...
    def rsa_public_check_sign(self,text,sign):
        ...
        verifier = PKCS1_signature.new(DIR_NAME/publick_key)
        digest = SHA.new()
...
def generate_public_rsa():
    ...
    with open(DIR_NAME+'/public_key.keystore','w')as f:
        f.write('-----BEGIN PUBLIC KEY-----\n')
        f.write(res_json['message']+'\n')
        f.write('-----END PUBLIC KEY-----')

    data = RSA.importKey(open(DIR_NAME+'/public_key.keystore').read())
    return data
...
if __name__ == '__main__':
    ...
    user = RsaEncrypt('public_key.keystore').encrypt_data('op13')
    ...
    print(data)
    # res = requests.post(url='https://192.168.4.220:8440/login/userLogin', json=data, verify=False)
    res = requests.post(url='https://192.168.4.220:8440/login/userLogin', json=data, verify=False)
    result =res.json()
    print(result)
