...
from zk_dev2.test.other_py.apiautotest.config.config import host, IP
...
if __name__ == '__main__':
    ...
    user = RsaEncrypt('public_key.keystore').encrypt_data('operator')
    pas = RsaEncrypt('public_key.keystore').encrypt_data('Admin@123456')
    print(user)
    print(pas)
    data = {"userName": user, "userPassword": pas}
    res = requests.post(url='https://192.168.7.208:8440/login/userLogin', json=data, verify=False)
